...
class VMUtilsV2(vmutils.VMUtils):
    ...
    def _create_vm_obj(self, vm_name, vnuma_enabled, vm_gen,
        ...
        if not vm_path and job:
            query = ("SELECT * FROM Msvm_AffectedJobElement "
                     "WHERE AffectingElement='%s'" % job_path)
            vm_path = self._conn.query(query).AffectedElement[0]
    ...
    def _drive_to_boot_source(self, drive_path):
        ...
        logical_identities_list = self._conn.query(
            "SELECT * FROM Msvm_LogicalIdentity")
        rasd_path = drive.path_() if is_physical else drive.Parent
        rasd_path = rasd_path.upper()
        for logical_identity in logical_identities_list:
            if rasd_path == logical_identity.SystemElement.upper():
                bssd_path = logical_identity.SameElement
                break
